...
- 2026-08-30: Deployment is blocked on Windows elevation. The direct copy was denied before changing Program Files, and the first `sudo` invocation rejected incompatible flags before elevation. Per the two-attempt stop rule, no third attempt was made. Installed OBS and all five critical binaries remain byte-identical to the backup; the hash-guarded one-shot installer is `L:\Coding\_turnover\obs-deploy-review-remediation.ps1`.
