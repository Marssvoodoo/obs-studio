...
static void process_packets(struct mp4_mux *mux, struct mp4_track *track, uint64_t *mdat_size)
{
	...
	/* The chapter track's last packet is the end-of-file "Dummy" marker that only exists to give the previous
	 * chapter its duration, so it is never emitted. */
	const size_t end_offset = (final_flush && track != mux->chapter_track) ? 0 : 1;
