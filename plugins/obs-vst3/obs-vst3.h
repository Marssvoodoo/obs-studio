...
#define SIDECHAIN_QUEUE_BLOCKS 32
#define MAX_VST3_STATE_BYTES (16U * 1024U * 1024U)
...
struct vst3_audio_data {
	...
	std::atomic<bool> process_failure_pending{false};
	std::atomic_flag init_in_progress = ATOMIC_FLAG_INIT;
	std::atomic<uint32_t> active_audio_calls{0};
	std::atomic<uint32_t> active_sidechain_calls{0};
