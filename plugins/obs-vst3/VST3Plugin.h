...
class VST3Plugin : public QObject {
	Q_OBJECT
public:
	static constexpr int32 MAX_HOSTED_PARAMETERS = 4096;
	...
	std::atomic<struct vst3_audio_data *> obsVst3Data{nullptr};
	...
	bool setBusActive(MediaType type, BusDirection direction, int which, bool active) const;
	bool init(const std::string &classId, const std::string &path, int sampleRate, int maxBlockSize,
		  SpeakerArrangement arr);
	bool activateComponent() const noexcept;
	void deactivateComponent() const;
	uint32 latencySamplesSafe() const noexcept;

	bool setProcessing(bool processing) const noexcept;
	...
	std::atomic<bool> parameterRefreshScheduled{false};
	void drainDspToGui();
